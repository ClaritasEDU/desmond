...
- **⭐ One-shot full exporter** (`desmond_export.py` + `desmond_export.sh`) — a
  ...
  open instantly. It's the headline path (web guide + README lead with it);
...
- **Summary:** Built the iMessage **attachment archiver** (real photos/videos/files
  ...
  **pagination (100/page)** so huge histories don't crash the browser. Four test
  suites (76 checks total, all passing); docs updated.
